      ...
      <DropdownToggle href='/' tag='a' className='nav-link dropdown-user-link' onClick={e => e.preventDefault()}>
        <div className='user-nav d-sm-flex d-none'>
          ...
          {ability.can('manage', 'ACL') ? (
            <span className='user-status'>client</span>
          ) : <span className='user-status'>admin</span>}
          {/* {ability.can('read', 'all') ? (
            <span className='user-status'>admind</span>
          ) : null} */}
          {/* { userData && userData.isAdmin === 'true' ? (<span className='user-status'>Admin1</span>) : (<span className='user-status'>Client1</span>) } */}
        </div>
